...
class DNC(object):
    ...
    Based on: https://github.com/claymcleod/tf-differentiable-neural-computer
    http://go.nature.com/2dIULo5
    """
    def __init__(
        self,
        X_train,
    ...
    def compile(self):
        ...
        self.read_strengths = tf.fill([R], 1e-6, name="free_gates_0")
        ...
        self.precedence = tf.fill([N], 1e-6, name="precedence_0")
        ...
        with tf.variable_scope("final"):
            for i in range(self.seq_output_len):
                ...
                loss_t = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=pred_t, labels=self.input_y[:, i]))
            ...
            self.opt_fn = optimize_loss(self.loss_fn, None, self.learning_rate, self.optimizer, clip_gradients=self.clip_gradients, summaries=OPTIMIZER_SUMMARIES)
    ...
    def get_reads(self, interface_out, i):
        with tf.variable_scope("interface_ops"):
            ...
            with tf.variable_scope("parse_reads"):
                read_keys, read_strengths, write_key, write_strength, erase_vector, write_vector, free_gates, allocation_gate, write_gate, read_modes = self.parse_interface(interface_out, i)

            # Content lookups
            with tf.variable_scope("content_lookup"):
                read_content_lookups = tf.stack([content_lookup(self.memory, K, b) for (K, b) in zip(tf.unstack(read_keys), tf.unstack(read_strengths))])
            ...
            # Update write weightings
            with tf.variable_scope("write_weighting"):
                # (1) get retention vector
                with tf.variable_scope("retention_vector"):
                    retention_vector = tf.reduce_prod(tf.stack([1 - f*wr for (f, wr) in zip(
                        tf.unstack(self.free_gates),
                        tf.unstack(self.read_weightings)
                    )]), reduction_indices=[0])

                # (2) update usage vector
                with tf.variable_scope("usage_vector"):
                    self.usage_vector = tf.multiply(self.usage_vector + self.write_weighting - tf.multiply(self.usage_vector, self.write_weighting), retention_vector)
                ...
                with tf.variable_scope("allocation_weightings"):
                    ...
                    allocation_vector_list = tf.squeeze(tf.stack(allocation_vector_list))

                # (4) update write weightings
                wg = self.write_gate
                ag = self.allocation_gate
                aw = allocation_vector_list
                ...
                for i in range(self.N):
                    ...
                    for j in range(self.N):
                        if i == j:
                            ...
                            this_row.append((1 - self.write_weighting[i] - self.write_weighting[j]) * self.linkage_matrix[i][j])
    ...
    def train(self, iterations=100, save_every_n_batches=750):
        ...
        while n_iter < iterations:
            ...
            for(batch_x, batch_y) in make_batches(self.X_train, self.Y_train, batch_size=self.batch_size):
                # Run graph
                [_, loss, rks, wks, ags, fgs, wgs, summaries, reads] = self.session.run([
                    self.opt_fn,
                    self.loss_fn,
                    self.read_keys_list,
                    self.write_keys_list,
                ...
                progress_bar = ""
                print("\rIteration {}: {} {}/{} ({:.2f}%) Loss: {:.5f}".format(n_iter+1, progress_bar, i, self.n_train_instances, percent*100.0, loss), end="")
                i += self.batch_size

                if i % save_every_n_batches == 0 and hasattr(self, "checkpoint_file_path"):
                    self.saver.save(self.session, self.checkpoint_file_path)

            # Asses at the end of iteration
